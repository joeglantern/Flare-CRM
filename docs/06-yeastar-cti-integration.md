...
| Subscribe events                  | `WSS /subscribe?access_token=` then send `{"topic_list":[…]}`                                 | text `heartbeat` every ≤ 60 s → `heartbeat response`                                                       | events as JSON text frames `{ type, sn, msg }`, where `msg` is a JSON **string**, decoded by `unwrapFrame` before any schema sees it |
