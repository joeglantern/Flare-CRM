...
- Whoever passed a task on can still see it in `own` scope (`assigned_by_id = me`); in that scope only the assignee can change it, under the usual write guard.
- Agents pass on only tasks they hold, made, or that nobody holds, whatever the visibility setting. With `team` or `all` visibility an agent can edit a colleague's visible task but not reassign it (403, and skipped in bulk assign). Admins and managers reassign any task they can see.
- An agent cannot unassign a task someone else gave them (409); they hand it back with a reason instead.
- A reassignment that takes the task out of the caller's own view still saves, answers with the task, and notifies the new assignee.
- `POST /api/v1/tasks/:id/decline { note }` hands the task back. Only the current assignee may call it, the reason is required (1 to 2000 characters), and the task must be open or in progress. It returns to `assigned_by_id`, or to `created_by_id` only for tasks with no assignment history at all (rows from before assigners were recorded). A task nobody else gave you (self-created, or one you took for yourself) cannot be handed back (409), nor can one whose assigner has been deactivated (409; a manager reassigns it). The task DTO's `handBackTo` names who it would return to, and is null in those cases, so the app hides the action.
- The task stays open with the returnee, whose own assigner is restored from the history, so a second hand-back unwinds one more step. The restored assigner is never the person handing back, so two people cannot bounce a task between them. A task completed or cancelled meanwhile is not reopened (the move only applies to an open or in-progress task).
