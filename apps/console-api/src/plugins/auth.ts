/**
 * Mounts Better Auth under /api/auth/* and exposes `app.auth` + `app.getSession()` (docs/21).
 * Sign-in failures are audited here because Better Auth's `after` hooks only run on success.
 * Copied from apps/api and trimmed to the actions this console actually has.
 */
...
export default fp(
  function authPlugin(app: FastifyInstance) {
    ...
    // Creating an owner and emailing them a set-password link. The password set here is random
